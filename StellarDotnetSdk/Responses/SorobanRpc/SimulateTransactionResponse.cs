...
public class SimulateTransactionResponse
{
    ...
    /// <remarks>
    ///     <para>
    ///         Stellar RPC tags the field <c>json:"minResourceFee,string,omitempty"</c>, so when present it
    ///         arrives as a JSON <em>string</em> rather than a number.
    ///         <see cref="JsonNumberHandlingAttribute" /> records that at the declaration instead of leaving it
    ///         to <see cref="JsonOptions.DefaultOptions" />'s ambient
    ///         <see cref="JsonNumberHandling.AllowReadingFromString" />. This is a public response type with
    ///         public <c>init</c> accessors, so a caller can deserialize it with their own options — and one
    ///         whose options bind the wire name but not the number handling (for example
    ...
    ///         <em>or</em> a bare number, so nothing that deserialized before is rejected. It is not the
    ///         read-side analogue of <see cref="JsonRequiredAttribute" />, which rejects an absent field.
    ///     </para>
    ///     <para>
    ///         A property-level number-handling attribute replaces the ambient setting in <em>both</em>
    ///         directions, so it also fixes the write shape: this property is always written as a bare number,
    ///         even for a caller whose options set <see cref="JsonNumberHandling.WriteAsString" />, and such a
    ///         caller can no longer opt these two properties into <see cref="JsonNumberHandling.Strict" />
    ///         reads. That is accepted deliberately — the alternative, adding
    ///         <see cref="JsonNumberHandling.WriteAsString" /> here, would change the serialized shape for
    ///         every caller rather than for the few who configure that flag.
    ///     </para>
    /// </remarks>
    ...
    public class RestorePreamble
    {
        ...
        ///     </para>
        ///     <para>
        ///         The <c>,string</c> in the tag quoted above is not decoration: the value arrives as a JSON
        ///         string, and the SDK read it only because <see cref="JsonOptions.DefaultOptions" /> sets
        ///         <see cref="JsonNumberHandling.AllowReadingFromString" /> globally. This is a public type with
        ///         public <c>init</c> accessors, so a caller whose own options bind the wire name but not the
        ///         number handling got a <see cref="System.Text.Json.JsonException" /> on a conforming reply —
        ///         independently of <c>required</c>, which governs an <em>absent</em> field rather than a
        ///         string-shaped one. <see cref="JsonNumberHandlingAttribute" /> records the wire form here so
        ///         it no longer depends on the ambient option; see the remarks on
        ///         <see cref="SimulateTransactionResponse.MinResourceFee" /> for its effect on the write
        ///         direction. It relaxes the read — a bare number still reads — so nothing that deserialized
        ///         before is rejected.
        ///     </para>
        /// </remarks>
        [JsonRequired]
