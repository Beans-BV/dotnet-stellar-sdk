...
/// </summary>
public class StellarRpcServer : IDisposable
{
    ...
    public StellarRpcServer(string uri, HttpClient httpClient)
    {
    ...
    public StellarRpcServer(string uri, string? bearerToken = null)
    {
        _serverUri = new Uri(uri);
    ...
    /// <param name="bearerToken">(Optional) Bearer token in case the server requires it.</param>
    public StellarRpcServer(string uri, HttpResilienceOptions? resilienceOptions, string? bearerToken = null)
    {
        _serverUri = new Uri(uri);
    ...
    /// <summary>
    ///     Releases resources used by this <see cref="StellarRpcServer" /> instance. If the HTTP client was created
    ...
    public static HttpClient CreateHttpClient(HttpMessageHandler handler)
    {
        ...
        var assembly = Assembly.GetAssembly(typeof(StellarRpcServer)).GetName();
    ...
    private async Task<TR> SendRequest<T, TR>(string method, T? parameters)
    {
        ...
        var httpContent = new StringContent(JsonSerializer.Serialize(sorobanRpcRequest, JsonOptions.DefaultOptions),
            Encoding.UTF8,
            "application/json");
