...
public class PaymentsRequestBuilderTest
{
    ...
    public void TestForLedger()
    {
        using (var server = new Server("https://horizon-testnet.stellar.org"))
        {
            var uri = server.Payments
                ...
                .BuildUri();
            Assert.AreEqual("https://horizon-testnet.stellar.org/ledgers/200000000000/payments?limit=50&order=asc",
                uri.ToString());
        }
    }
    ...
    {
        var json = await File.ReadAllTextAsync(Path.Combine("testdata/operations/payment", "payment.json"));
