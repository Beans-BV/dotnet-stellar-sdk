...
public class SimulateTransactionResponseDeserializerTest
{
    ...
    /// <summary>
    ///     Verifies that both <c>minResourceFee</c> fields read from a JSON <em>string</em> under options that do
    ///     not enable <see cref="JsonNumberHandling.AllowReadingFromString" /> globally. Stellar RPC tags both
    ///     <c>json:"minResourceFee,string"</c>, so a string is the conforming wire form; the SDK read it only
    ///     because <see cref="JsonOptions.DefaultOptions" /> sets that option for every type it handles. This is a
    ///     public response type with public <c>init</c> accessors, so a caller can deserialize it with their own
    ///     options — and there a conforming reply failed outright, which making the preamble's field
    ///     <c>required</c> turned from a silent <c>0</c> into a hard <see cref="JsonException" />. The
    ///     property-level attribute is what carries the wire contract to those callers.
    /// </summary>
    [TestMethod]
    public void Deserialize_WithStringMinResourceFee_UnderOptionsWithoutNumberHandling_ReturnsValue()
    {
        // Options a caller might reasonably build themselves: property names still match the wire, but nothing
        // grants the global number-from-string allowance that JsonOptions.DefaultOptions provides.
        var callerOptions = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
        const string json =
            """{"latestLedger":1,"minResourceFee":"58181","restorePreamble":{"transactionData":"AAAA","minResourceFee":"9999"}}""";

        var response = JsonSerializer.Deserialize<SimulateTransactionResponse>(json, callerOptions);

        Assert.AreEqual(58181L, response!.MinResourceFee);
        Assert.AreEqual(9999L, response.RestorePreambleInfo!.MinResourceFee);
    }

    /// <summary>
    ///     Verifies that accepting the string form does not stop a bare number from reading, and — the part that
    ///     can actually fail — that the allowance is scoped to the two fee properties. No value of
    ///     <see cref="JsonNumberHandling" /> makes a bare number fail to read into a <see cref="long" />, so the
    ///     first assertion alone would be a tautology. The <c>latestLedger</c> assertion is what pins the scope:
    ///     Stellar RPC tags that field <c>json:"latestLedger"</c> with no <c>,string</c>, so it is a bare number
    ///     on the wire and must stay strict under options that did not ask otherwise. Moving the attribute to the
    ///     type level would relax every number on the response and is caught here.
    /// </summary>
    [TestMethod]
    public void Deserialize_WithNumericMinResourceFee_UnderOptionsWithoutNumberHandling_ReturnsValue()
    {
        var callerOptions = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
        const string json =
            """{"latestLedger":1,"minResourceFee":58181,"restorePreamble":{"transactionData":"AAAA","minResourceFee":9999}}""";
        ...
        // Scope: latestLedger carries no allowance of its own, so a string still fails under these options.
        Assert.ThrowsException<JsonException>(() =>
            JsonSerializer.Deserialize<SimulateTransactionResponse>(
                """{"latestLedger":"1"}""", callerOptions));
    }
